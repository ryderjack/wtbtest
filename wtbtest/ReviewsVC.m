        ...
        if (objects) {
            ...
            if (objects.count == 0) {
                ...
                for (PFObject *review in objects) {
                    ...
                    if ([sellerId isEqualToString:self.user.objectId]) {
//                        NSLog(@"SOLD: %@", review);
                        [self.soldFeedback addObject:review];
                    }
                    else{
//                        NSLog(@"PURCHASED: %@", review);
            ...
            if (self.purchasedFeedback.count > 0 && self.soldFeedback.count > 0) {
                NSString *purchaseTitle = [NSString stringWithFormat:@"%lu  P U R C H A S E D", (unsigned long)self.purchasedFeedback.count];
                NSString *soldTitle = [NSString stringWithFormat:@"%lu  S O L D", (unsigned long)self.soldFeedback.count];

                [self.segmentedControl setSectionTitles:@[purchaseTitle,soldTitle]];
            }
            else if (self.purchasedFeedback.count == 0 && self.soldFeedback.count > 0){
                NSString *soldTitle = [NSString stringWithFormat:@"%lu  S O L D", (unsigned long)self.soldFeedback.count];
                
                [self.segmentedControl setSectionTitles:@[@"P U R C H A S E D",soldTitle]];
            }
            else if (self.purchasedFeedback.count > 0 && self.soldFeedback.count == 0){
                NSString *purchaseTitle = [NSString stringWithFormat:@"%lu  P U R C H A S E D", (unsigned long)self.purchasedFeedback.count];
                
                [self.segmentedControl setSectionTitles:@[purchaseTitle,@"S O L D"]];
            }
